...
// Rend un node Three.js quel que soit son type :
// - Mesh direct → un seul <mesh>
// - Group (multi-matériaux) → un <mesh> par enfant avec sa position locale
function RenderNode({ node, ...props }) {
  ...
  // Group avec plusieurs primitives (multi-matériaux Blender)
  if (node.type === "Group") {
    return (
      <group
        position={node.position}
...
export function AnnexZone({ cardStates, annexPhase, onCardHit, onQuestLost }) {
  ...
  return (
    <>
      {/* Sol multi-matériaux (Material.005 + green_poker) */}
      <RigidBody
        type="fixed"
        colliders={false}
        restitution={0.3}
